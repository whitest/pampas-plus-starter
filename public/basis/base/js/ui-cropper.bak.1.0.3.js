...
var MMCQ = (function() {
    ...
    CMap.prototype = {
        ...
        nearest: function(color) {
            ...
            for (var i = 0; i < vboxes.size(); i++) {
                d2 = Math.sqrt(
                    Math.pow(color[0] - vboxes.peek(i)
                        .color[0], 2) +
                    Math.pow(color[1] - vboxes.peek(i)
                        .color[1], 2) +
                    Math.pow(color[2] - vboxes.peek(i)
                        .color[2], 2)
                );
                if (d2 < d1 || d1 === undefined) {
                    d1 = d2;
                    pColor = vboxes.peek(i)
                        .color;
                }
            }
            return pColor;
...
crop.factory('cropAreaCircle', ['cropArea', function(CropArea) {
    ...
    CropAreaCircle.prototype._calcCirclePerimeterCoords = function(angleDegrees) {
        ...
        var angleRadians = angleDegrees * (Math.PI / 180),
            circlePerimeterX = this.getCenterPoint()
            .x + hSize * Math.cos(angleRadians),
            circlePerimeterY = this.getCenterPoint()
            .y + hSize * Math.sin(angleRadians);
    ...
    CropAreaCircle.prototype._isCoordWithinArea = function(coord) {
        return Math.sqrt((coord[0] - this.getCenterPoint()
            .x) * (coord[0] - this.getCenterPoint()
            .x) + (coord[1] - this.getCenterPoint()
            .y) * (coord[1] - this.getCenterPoint()
            .y)) < this._size.w / 2;
    };
    ...
    CropAreaCircle.prototype.processMouseMove = function(mouseCurX, mouseCurY) {
        ...
        if (this._areaIsDragging) {
            ...
            newNO.x = this.getCenterPoint()
                .x - iFR * 0.5;
            newSE.x = this.getCenterPoint()
                .x + iFR * 0.5;

            newNO.y = this.getCenterPoint()
                .y - iFR * 0.5;
            newSE.y = this.getCenterPoint()
                .y + iFR * 0.5;
        ...
        angular.element(this._ctx.canvas)
            .css({
                'cursor': cursor
            });
...
crop.factory('cropArea', ['cropCanvas', function(CropCanvas) {
    ...
    CropArea.prototype.setInitCoords = function(coords) {
        ...
        coords.h = this.getSize()
            .h;
        coords.w = this.getSize()
            .w;
    ...
    CropArea.prototype._processSize = function(size) {
        ...
        return {
            x: (typeof size.x === 'undefined') ? this.getSize()
                .x : size.x,
            y: (typeof size.y === 'undefined') ? this.getSize()
                .y : size.y,
...
crop.factory('cropHost', ['$document', '$q', 'cropAreaCircle', 'cropAreaSquare', 'cropAreaRectangle', 'cropEXIF', function($document, $q, CropAreaCircle, CropAreaSquare, CropAreaRectangle, cropEXIF) {
    ...
    return function(elCanvas, opts, events) {
        ...
        var resetCropHost = function() {
            if (image !== null) {
                ...
                if (initMax) {
                    ...
                    theArea.setSize({
                        w: Math.min(theArea.getInitSize()
                            .w, cw / 2),
                        h: Math.min(theArea.getInitSize()
                            .h, ch / 2)
                    });
                } else {
                    theArea.setSize({
                ...
                if (theArea.getInitCoords()) {
                    if (self.areaInitIsRelativeToImage) {
                        ...
                        theArea.setSize({
                            w: theArea.getInitSize()
                                .w / ratio,
                            h: theArea.getInitSize()
                                .h / ratio,
                            x: theArea.getInitCoords()
                                .x / ratio,
                            y: theArea.getInitCoords()
                                .y / ratio
                        });
                    } else {
                        theArea.setSize({
        ...
        var renderImageToDataURL = function(getResultImageSize) {
            ...
            if (image !== null) {
                var x = (center.x - theArea.getSize()
                        .w / 2) * (image.width / ctx.canvas.width),
                    y = (center.y - theArea.getSize()
                        .h / 2) * (image.height / ctx.canvas.height),
                    areaWidth = theArea.getSize()
                    .w * (image.width / ctx.canvas.width),
                    areaHeight = theArea.getSize()
                    .h * (image.height / ctx.canvas.height);
        ...
        this.getResultImage = function() {
            ...
            for (var i = 0; i < resImgSizeArray.length; i++) {
                arrayResultImages.push({
                    dataURI: renderImageToDataURL(resImgSizeArray[i])
                        .dataURI,
        ...
        this.setNewImageSource = function(imageSource) {
            ...
            if (!!imageSource) {
                ...
                if (imageSource instanceof window.Blob) {
                    ...
                    if (imageSource.substring(0, 4)
                        .toLowerCase() === 'http' || imageSource.substring(0, 2) === '//') {
        ...
        this.getResultImageSize = function() {
            ...
            if (resImgSize === 'max') {
                ...
                var size = {
                    w: zoom * theArea.getSize()
                        .w,
                    h: zoom * theArea.getSize()
                        .h
                };
        ...
        link: function(scope, element) {
            ...
            var updateResultImage = function(scope, force, callback) {
                if (scope.image !== '' && (!scope.liveView.block || force)) {
                    ...
                    if (storedResultImage !== resultImage) {
                        ...
                        cropHost.getResultImageDataBlob()
                            .then(function(blob) {
                        ...
                        if (scope.resultImage) {
                            cropHost.getDominantColor(scope.resultImage)
                                .then(function(dominantColor) {
                                    scope.dominantColor = dominantColor;
                                });
                            cropHost.getPalette(scope.resultImage)
            ...
            var updateCropject = function(scope) {
                ...
                var dimRatio = {
                    x: cropHost.getArea()
                        .getImage()
                        .width / cropHost.getArea()
                        .getCanvasSize()
                        .w,
                    y: cropHost.getArea()
                        .getImage()
                        .height / cropHost.getArea()
                        .getCanvasSize()
                        .h
                };

                scope.cropject = {
                    canvasSize: cropHost.getArea()
                        .getCanvasSize(),
